...
export function FlowTimeline({ milestones = [] }) {
  return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', position: 'relative', paddingLeft: 40 } },
    ...
    milestones.map((m, i) => React.createElement('div', { key: i, style: { position: 'relative', paddingBottom: i < milestones.length - 1 ? 40 : 0 } },
      ...
      React.createElement('div', { style: { fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: var(--text-body-size), color: 'var(--text-title)' } }, m.label),
      m.description && React.createElement('div', { style: { fontFamily: 'var(--font-body)', fontSize: var(--text-caption-size), lineHeight: '32px', color: 'var(--text-secondary)', marginTop: 8 } }, m.description)
    ))
  );
}
