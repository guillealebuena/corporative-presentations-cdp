...
export function KpiRing({ percent = 0, size = 160, color = 'var(--brand-primary)' }) {
  ...
  return React.createElement('div', { style: { position: 'relative', width: size, height: size } },
    ...
    React.createElement('div', { style: { position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: var(--text-body-lead-size), color: 'var(--text-title)' } }, percent + '%')
  );
}
