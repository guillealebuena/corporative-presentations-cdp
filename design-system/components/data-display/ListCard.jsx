...
export function ListCard({ title, icon = 'ListBullets', rows = [], width = 410, height = 260 }) {
  return React.createElement('div', {
    ...
    title && React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 12 } },
      ...
      React.createElement('div', { style: { fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: var(--text-caption-size), color: 'var(--text-title)' } }, title)
    ),
    React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 12 } },
      rows.map((r, i) => React.createElement('div', {
        key: i,
        style: { display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'space-between', fontFamily: 'var(--font-body)', fontSize: var(--text-caption-size), color: 'var(--text-body)', opacity: Math.max(1 - i * .18, .4) }
      },
        React.createElement('span', { style: { display: 'flex', alignItems: 'center', gap: 12 } },
